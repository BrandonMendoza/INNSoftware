<template>
  <el-form :model="currentEmpleado" ref="EmpleadosSueldosForm" label-position="top"  label-width="150px">
    <el-dialog
    :ref="dialogRef"
    id="dialogoHistorialSueldo"
    :title="dialogTitle"
    width="40%"
    :visible.sync="dialogoHistorialSueldo">
      ...
      <el-tabs v-model="activeName" :stretch="true">
        <el-tab-pane label="Historial" name="first">
          <el-row type="flex" justify="space-around" id="embarqueTransfer">
            <el-col>
              <el-table
              :data="historialSueldoEmpleado"
              ref="historialSueldoList"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogoHistorialSueldo = false;" class="float-left">Cerrar</el-button>
      </span>
    </el-dialog>
...
import HistorialSueldoEmpleado from '@/api/historialSueldoEmpleado';

const historialSueldoEmpleadoResource = new HistorialSueldoEmpleado('historialSueldoEmpleado');
  ...
  export default {
    data() {
      return {
        ...
        /*Dialog*/
        dialogoHistorialSueldo:false,
        dialogTitle: '',
        dialogRef: 'EmpleadosHistorialSueldoDialog',
    ...
    methods: {
      ...
      /* METODOS PRINCIPALES */
      async getList(){
        const { data, meta } = await historialSueldoEmpleadoResource.historialSueldoByEmpleado(this.currentEmpleado.id);
        console.log("DATA GET LIST");
        console.log(data);
        this.historialSueldoEmpleado = data;
      },
      /*DIALOG METHODS*/
      open() {
        this.dialogoHistorialSueldo = true;
        this.currentEmpleado = this.$parent.current;
        this.dialogTitle = 'Historial Sueldos: ('+this.currentEmpleado.numero_empleado+') '+this.currentEmpleado.nombre+' '+this.currentEmpleado.apellidos;
        this.getList();
      },
      close() {
